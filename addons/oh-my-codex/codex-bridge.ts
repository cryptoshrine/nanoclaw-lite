 ...
 *
 * ADDON: oh-my-codex
 * Install: Copy this file to src/codex-bridge.ts and add config entries.
 * See addons/oh-my-codex/addon.json for details.
 */
...
// ── Configuration (add these to config.ts or set via env) ────────────────────

const OMX_RUNTIME_CLI_PATH = process.env.OMX_RUNTIME_CLI_PATH || '';
const OMX_CODEX_JOBS_DIR = process.env.OMX_CODEX_JOBS_DIR ||
...
export async function startCodexJob(params: {
  ...
  if (!OMX_RUNTIME_CLI_PATH || !fs.existsSync(OMX_RUNTIME_CLI_PATH)) {
    throw new Error(
      `OMC runtime-cli.js not found at "${OMX_RUNTIME_CLI_PATH}". ` +
      `Set OMX_RUNTIME_CLI_PATH in .env and run 'npm run build' in the OMC project.`,
    );
  }
 ...
 * Sends a continuation message to each active pane.
 */
export function nudgeCodexJob(
  ...
  try {
    if (!fs.existsSync(panesPath)) return false;
    ...
    if (!panes.paneIds || panes.paneIds.length === 0) return false;
...
/**
 * Check if a string is a Codex job ID.
 */

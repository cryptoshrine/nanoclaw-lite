...
export type TmuxAgentType = 'codex' | 'claude-cli' | 'gemini' | 'custom';
...
export interface TmuxSpawnConfig {
  ...
  claudeModel?: string;
  /** For gemini: model override (default: gemini-2.5-pro) */
  geminiModel?: string;
}
...
function buildWorkerCommand(config: TmuxSpawnConfig): string {
  ...
  switch (config.agentType) {
    ...
    case 'gemini': {
      const model = config.geminiModel || 'gemini-2.5-pro';
      return `gemini --model ${model} --sandbox none '${escapedPrompt}'`;
    }
    case 'custom':
